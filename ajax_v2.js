...
function _ajax_post(action, o){
	...
	if(_post['URL']){ //call based
 	...
 	if(action){
		...
		formData.append('_ajax_action', action);
	}

	for (var i in _post) if(i!='URL'){
	 	value=_post[i];
		formData.append(i, _post[i]);
	 }
	_post = new Object();
	Xajax(url,'Xfill');
}
...
function _upload_file(id_file) {
	inputFile = document.getElementById(id_file);
	formData.append(inputFile.getAttribute('name'), inputFile.files[0]);
}

function Xajax(url,xfunc){
	...
	.then((responseText) => {
		eval(xfunc+'(\''+escape(responseText)+'\');');
